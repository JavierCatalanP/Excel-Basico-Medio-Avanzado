# Bienvendios a las practicas del curso "Universidad Excel" en Udemy
# Excel-Básico - Medio - Avanzado
Practicas sacada del curso en Udemy, link del curso: https://www.udemy.com/share/103uHG3@6uD81nfZqET-1SzX4V92jHF_9MtKHRY6lUUpexzHQ_HdoJA3wmUftGf_iEB_90Sz/ 
<div id="header" align="center">
<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.demolab.com?font=Fira+Code&pause=1000&color=EA4E43&random=false&width=435&lines=Curso+ Impartido+por+Ubaldo+Acosta%3A" alt="Typing SVG" /></a>
 </div>

Lo que aprenderás
- Aprenderán a utilizar Microsoft Excel desde Cero hasta Avanzado
- Aprenderán temas como: Libros, Hojas, Celdas, Columnas y Renglones (Filas)
- Uso de Listas y Tablas
- Atajos en Excel
- Creación de Gráficos
- Manejo de Fórmulas y Funciones desde básico hasta avanzado
- Consolidación de Datos
- Impresión de Datos
- Tablas Dinámicas en Excel
- Power Pivot, Power View y Power Map
- Macros y VBA en Excel
Y muchísimos temas más

<div id="header" align="center">
  <img src="[[https://d1jnx9ba8s6j9r.cloudfront.net/blog/wp-content/uploads/2016/11/Git-Architechture-Git-Tutorial-Edureka-2.png](https://www.cediformacio.com/wp-content/uploads/2021/07/curso-Excel-inicio-medio-avanzado.jpg)](https://tse2.mm.bing.net/th?id=OIP.19gUcUFg_j0hHcIylxSwfwHaDh&pid=Api&P=0&h=180)" width="300" /></br>
</div>

##Master en Excel más completo. Incluye Funciones, Tablas Dinámicas, Gráficos, Power Query , Macros y más a profundidad!
```
Formulas aquí
```
El temario de nuestro curso incluye:

- Introducción a Excel
```
Formulas aquí
```
- Qué es Excel, como abrirlo, Interfaz General de Excel y configuración básica
```
Formulas aquí
```
- Interfaz General
```
Formulas aquí
```
- Barra de Acceso Rápido
```
Formulas aquí
```
- Barra de Opciones
```
Formulas aquí
```
- Ocultar la barra de opciones y volver a mostrarla
```
Formulas aquí
```
- Menú Archivo
```
Formulas aquí
```
- Libros, Hojas, Celdas, Columnas y Renglones:
```
Formulas aquí
```
- Libro: Crear, Abrir y Guardar
```
Formulas aquí
```
- Hojas
```
Formulas aquí
```
- Celdas, Columnas y Renglones
```
Formulas aquí
```
- Qué es una celda en Excel
```
Formulas aquí
```
- Tipo de Información que almacena una celda
```
Formulas aquí
```
- Texto, Números, Fechas, Formulas y Funciones
```
Formulas aquí
```
-Formato de Celdas:
```
Formulas aquí
```
- Atajos en Excel
```
Formulas aquí
```
- Listas (Rangos)
```
Formulas aquí
```
- Ordenamiento por columna
```
Formulas aquí
```
- Filtrar (sin convertir a tabla)
```
Formulas aquí
```
- Agrupar (columnas o renglones)
```
Formulas aquí
```
- Buscar y reemplazar información
```
Formulas aquí
```
- Inmovilizar paneles
```
Formulas aquí
```
- Tablas
```
Formulas aquí
```
- Segmentación de Datos
```
Formulas aquí
```
- Vistas
```
Formulas aquí
```
- Manejo de Ventanas (Visualizar varios archivos a la vez)
```
Formulas aquí
```
- Gráficos
```
Formulas aquí
```
- Diseño de Gráficos
```
Formulas aquí
```
- Minigráficos
```
Formulas aquí
```
- Ilustraciones
```
Formulas aquí
```
- Imágenes e ilustraciones
```
Formulas aquí
```
- Smart Art
```
Formulas aquí
```
- Fórmulas
```
Formulas aquí
```
- Barra de Fórmula:
```
Formulas aquí
```
- Nombres de Rangos
```
Formulas aquí
```
- Referencias relativas y absolutas
```
Formulas aquí
```
- Referencias entre hojas
```
Formulas aquí
```
- Referencias entre libros
```
Formulas aquí
```
- Constantes
```
Formulas aquí
```
- Crear una constante y usarla como dato en nuestras formulas
```
Formulas aquí
```
- Tipos de Operadores en Excel
```
Formulas aquí
```
Operaciones Matemáticas Básicas en Excel
```
Formulas aquí
```
Precedencia o Prioridad de Operadores en Excel
```
Formulas aquí
```
- Funciones
```
Formulas aquí
```
- Función concat y concatenar
```
Formulas aquí
```
- Sumar celdas sin uso de funciones
```
Formulas aquí
```
- Otras funciones básicas:
```
Formulas aquí
```
- count
```
Formulas aquí
```
- promedio
```
Formulas aquí
```
- min
```
Formulas aquí
```
- max
```
Formulas aquí
```
- contar números
```
Formulas aquí
```
- Ejercicio, calculadora con Excel
```
Formulas aquí
```
- Crear las operaciones de Sumar, Restar, Multiplicar, Dividir,
```
Formulas aquí
```
- Función de autocompletado de secuencias o listas
```
Formulas aquí
```
- Función buscarv y buscarh
```
Formulas aquí
```
- Formato Condicional
```
Formulas aquí
```
- Tablas Avanzado
```
Formulas aquí
```
-  Validar celda con una lista de datos
```
Formulas aquí
```
- Función Subtotales
```
Formulas aquí
```
- Subtotales
```
Formulas aquí
```
- Consolidación de Datos
```
Formulas aquí
```
- Plantillas
```
Formulas aquí
```
- Impresión
```
Formulas aquí
```
- Vista -> Diseño Pagina -> Cabecero y Pie Página
```
Formulas aquí
```
- Imprimir solo una parte de la hoja
```
Formulas aquí
```
- Imprimir el encabezado en varias páginas
```
Formulas aquí
```
- Tablas dinámicas
```
Formulas aquí
```
- Resumen de información
```
Formulas aquí
```
- Gráficos de Tablas dinámicas
```
Formulas aquí
```
- Power Query
```
Formulas aquí
```
- Macros
```
Formulas aquí
```
- Macros en Excel
```
Formulas aquí
```
- Proyectos del Mundo real en Excel
```
Formulas aquí
```
Y mucho más, así que te esperamos del otro lado para que te conviertas en un Experto en Excel.
